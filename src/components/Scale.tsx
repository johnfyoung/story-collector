...
export function Scale({ label, max, value, onChange }: { label: string; max: number; value: number | string | undefined; onChange: (v: number) => void }) {
  ...
  return (
    <div>
      ...
      <div style={wrap}>
        {items.map((i) => (
          <div key={i} title={String(i)} style={dot(i)} onClick={() => onChange(i)} />
        ))}
