import { Databases, Models, Storage, Permission, Role } from 'appwrite'
...
const STORIES_COLLECTION_ID = import.meta.env.VITE_APPWRITE_STORIES_COLLECTION_ID as string | undefined
const BUCKET_ID = import.meta.env.VITE_APPWRITE_BUCKET_STORIES as string | undefined
...
export class StoriesRemote {
  ...
  async listWithFiles(): Promise<{ stories: Story[]; files: Record<string, string> }> {
    if (!DB_ID || !STORIES_COLLECTION_ID) throw new Error('Remote stories not configured')
  ...
  async createWithFile(meta: Omit<Story, 'id'>, content: StoryContent): Promise<{ story: Story; fileId: string }> {
    if (!DB_ID || !STORIES_COLLECTION_ID || !BUCKET_ID) throw new Error('Remote stories not configured')
    const filename = StoriesRemote.makeSafeFilename(meta.name)
    const fileObj = new File([JSON.stringify(content, null, 2)], filename, { type: 'application/json' })
    const file = await this.storage.createFile(BUCKET_ID, ID.unique(), fileObj, this.permsOwner())
    const doc = await this.db.createDocument(DB_ID, STORIES_COLLECTION_ID, ID.unique(), {
      ...meta,
      jsonFileId: file.$id,
    } as any, this.permsOwner())
    return { story: toStory(doc), fileId: file.$id }
  }

  async update(id: string, patch: Partial<Omit<Story, 'id'>>): Promise<Story> {
    if (!DB_ID || !STORIES_COLLECTION_ID) throw new Error('Remote stories not configured')
    const doc = await this.db.updateDocument(DB_ID, STORIES_COLLECTION_ID, id, patch as any)
    return toStory(doc)
  }

  async saveJson(storyId: string, data: StoryContent, currentFileId?: string): Promise<string> {
    if (!DB_ID || !STORIES_COLLECTION_ID || !BUCKET_ID) throw new Error('Remote stories not configured')
    const fileObj = new File([JSON.stringify(data, null, 2)], `${storyId}.json`, { type: 'application/json' })
    const file = await this.storage.createFile(BUCKET_ID, ID.unique(), fileObj, this.permsOwner())
    await this.db.updateDocument(DB_ID, STORIES_COLLECTION_ID, storyId, { jsonFileId: file.$id } as any)
  ...
  async readJson(fileId: string): Promise<StoryContent> {
    ...
    const urlOrString = this.storage.getFileDownload(BUCKET_ID, fileId) as unknown as string | URL
    const href = typeof urlOrString === 'string' ? urlOrString : urlOrString.href
  ...
  subscribeDocs(handler: (type: 'create' | 'update' | 'delete', doc: Story) => void) {
    if (!DB_ID || !STORIES_COLLECTION_ID) throw new Error('Remote stories not configured')
    ...
    const unsub = client.subscribe(channel, (resp: any) => {
      ...
      else if (events.some((e) => e.endsWith('.update'))) handler('update', story)
      else if (events.some((e) => e.endsWith('.delete'))) handler('delete', story)
    })
    return unsub
  }

  subscribeFiles(handler: (type: 'create' | 'update' | 'delete', fileId: string) => void) {
    if (!BUCKET_ID) throw new Error('Remote stories not configured')
    const channel = `buckets.${BUCKET_ID}.files`
    const unsub = client.subscribe(channel, (resp: any) => {
      const events: string[] = resp.events || []
      const payload = resp.payload as { $id: string }
      if (events.some((e) => e.endsWith('.create'))) handler('create', payload.$id)
      else if (events.some((e) => e.endsWith('.update'))) handler('update', payload.$id)
      else if (events.some((e) => e.endsWith('.delete'))) handler('delete', payload.$id)
    })
    return unsub
  }
}
