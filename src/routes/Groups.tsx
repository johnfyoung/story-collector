...
export default function Groups() {
  ...
  async function remove(id: string) {
    ...
    const next: StoryContent = {
      ...content,
      groups: content.groups.filter((x) => x.id !== id),
    };
  ...
  return (
    <div style={{ display: "grid", gap: 16 }}>
      ...
      >
        ...
        {storyId ? (
          <a
            href={`/stories/${storyId}/groups/new`}
      ...
      {content ? (
        <SearchBox
          value={query}
          onChange={setQuery}
          suggestions={content.groups.map((e) => e.name)}
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((el) => (
              <Card
                ...
                onClick={() =>
                  storyId &&
                  navigate(`/stories/${storyId}/groups/${el.id}/edit`)
                }
              >
                <div
                  style={{
