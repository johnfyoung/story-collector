...
export default function Languages() {
  ...
  const filtered = useMemo(() => {
    ...
    return content.languages.filter((el) => el.name.toLowerCase().includes(q) || (el.shortDescription ?? '').toLowerCase().includes(q))
  }, [content, query])
  ...
  async function remove(id: string) {
    ...
    if (!ok) return
    const next: StoryContent = { ...content, languages: content.languages.filter((x) => x.id !== id) }
  ...
  return (
    <div style={{ display: 'grid', gap: 16 }}>
      {storyId ? <TabNav active="languages" storyId={storyId} /> : null}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h1 style={{ margin: 0, color: 'var(--color-text)' }}>Languages</h1>
        {storyId ? (
          <a href={`/stories/${storyId}/languages/new`} style={{ textDecoration: 'none' }}>
            <Button>Add language</Button>
          </a>
        ) : null}
      </div>
      {content ? (
        <SearchBox value={query} onChange={setQuery} suggestions={content.languages.map((e) => e.name)} placeholder="Search languages…" />
      ) : null}
      {!content ? (
        ...
        <div style={{ display: 'grid', gap: 12 }}>
          {filtered.length === 0 ? (
            <Card>
              <div style={{ color: 'var(--color-text-muted)' }}>No languages match.</div>
            </Card>
          ) : (
            filtered.map((el) => (
              <Card key={el.id} onClick={() => storyId && navigate(`/stories/${storyId}/languages/${el.id}/edit`)}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    {storyId ? (
                      <a href={`/stories/${storyId}/languages/${el.id}/edit`} style={{ textDecoration: 'none' }} onClick={(e) => e.stopPropagation()}>
