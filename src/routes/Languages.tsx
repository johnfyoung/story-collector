...
export default function Languages() {
  ...
  const filtered = useMemo(() => {
    ...
    return content.languages.filter(
      (el) =>
  ...
  async function remove(id: string) {
    ...
    const next: StoryContent = {
      ...content,
      languages: content.languages.filter((x) => x.id !== id),
    };
  ...
  return (
    <div style={{ display: "grid", gap: 16 }}>
      ...
      >
        ...
        {storyId ? (
          <a
            href={`/stories/${storyId}/languages/new`}
      ...
      {content ? (
        <SearchBox
          value={query}
          onChange={setQuery}
          suggestions={content.languages.map((e) => e.name)}
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((el) => (
              <Card
                ...
                onClick={() =>
                  storyId &&
                  navigate(`/stories/${storyId}/languages/${el.id}/edit`)
                }
              >
                <div
                  style={{
