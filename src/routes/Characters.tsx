...
export default function Characters() {
  ...
  const filtered = useMemo(() => {
    ...
    return content.characters.filter((c) => c.name.toLowerCase().includes(q) || (c.shortDescription ?? '').toLowerCase().includes(q))
  }, [content, query])
  ...
  return (
    <div style={{ display: 'grid', gap: 16 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        {storyId ? (
          <a href={`/stories/${storyId}/characters/new`} style={{ textDecoration: 'none' }}>
      ...
      {!content ? (
        ...
        <div style={{ display: 'grid', gap: 12 }}>
          <SearchBox value={query} onChange={setQuery} suggestions={content.characters.map((c) => c.name)} placeholder="Search characters…" />
          {filtered.length === 0 ? (
            <Card>
              <div style={{ color: 'var(--color-text-muted)' }}>No characters match.</div>
            </Card>
          ) : (
            filtered.map((c) => (
              <Card key={c.id} onClick={() => storyId && navigate(`/stories/${storyId}/characters/${c.id}/edit`)}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <Avatar name={c.name} url={c.avatarUrl} size={40} editable={false} onChange={() => { /* no-op in list */ }} />
                    <div>
                      <div style={{ color: 'var(--color-text)', fontWeight: 600 }}>{c.name}</div>
                      {c.shortDescription ? (
                        <div style={{ color: 'var(--color-text-muted)', marginTop: 4 }}>{c.shortDescription}</div>
                      ) : null}
                    </div>
                  </div>
                  {storyId ? (
                    <a href={`/stories/${storyId}/characters/${c.id}/edit`} style={{ textDecoration: 'none' }} onClick={(e) => e.stopPropagation()}>
